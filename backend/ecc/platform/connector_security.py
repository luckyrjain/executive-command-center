...
This module started as a *foundation only*; tasks adopt each helper at its
call sites incrementally (adopted so far: conflict narrowing and the guarded,
safety-checked revoke sites -- Gmail callback, engineering disable, Gmail
revocation cascade; still pending: grant/transfer/delegation refusal, Gmail
callback identity binding, membership-removal lock key). Keeping the definitions in one
...
# resource_type -> fixed SQL returning one boolean row for `:id`. Every
# statement is a code-defined literal (no interpolation of caller input);
# an unknown resource_type never reaches SQL.
_PERSONAL_PREDICATES: Final[dict[str, str]] = {
    "connector_accounts": (
        "SELECT EXISTS (SELECT 1 FROM connector_accounts "
        "WHERE id = :id AND provider = ANY(:providers))"
    ),
    "sync_runs": (
        "SELECT EXISTS (SELECT 1 FROM sync_runs sr "
        "JOIN connector_accounts ca ON ca.workspace_id = sr.workspace_id "
        "AND ca.id = sr.connector_account_id "
        "WHERE sr.id = :id AND ca.provider = ANY(:providers))"
    ),
    "sync_cursors": (
        "SELECT EXISTS (SELECT 1 FROM sync_cursors sc "
        "JOIN connector_accounts ca ON ca.workspace_id = sc.workspace_id "
        "AND ca.id = sc.connector_account_id "
        "WHERE sc.id = :id AND ca.provider = ANY(:providers))"
    ),
    "attention_items": (
        "SELECT EXISTS (SELECT 1 FROM attention_items "
        "WHERE id = :id AND entity_type = :attention_entity_type)"
    ),
    "recommendations": (
        "SELECT EXISTS (SELECT 1 FROM recommendations "
        "WHERE id = :id AND recommendation_type = :recommendation_type)"
    ),
    "ai_runs": (
        "SELECT EXISTS (SELECT 1 FROM ai_runs WHERE id = :id AND task_type = ANY(:task_types))"
    ),
    "pkos_evidence": (
        "SELECT EXISTS (SELECT 1 FROM pkos_evidence "
        "WHERE id = :id AND source_type = :evidence_source_type)"
    ),
}
...
def membership_mutation_lock_key(workspace_id: UUID) -> str:
    ...
    for its exclusive membership-mutation lock. Callers taking the shared
