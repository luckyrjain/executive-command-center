"""`ecc.platform.authz` -- Phase 8 Task 3's authorization decision engine
...
**The grants/ownership-transfer HTTP feature (`GET|POST /sharing/grants`,
`DELETE /sharing/grants/{id}`, `POST|GET /ownership/transfers`) lives in
`authz_grants.py`, not here** -- an architecture-review deepening split
this module in two: this file is the decision engine 40+ callers across
...
def current_role(session: Session, *, workspace_id: UUID, users_id: UUID) -> Role | None:
...
def account_id_for(session: Session, *, workspace_id: UUID, users_id: UUID) -> UUID | None:
...
def load_resource(session: Session, *, resource_type: str, resource_id: UUID) -> ResourceRef | None:
    """Read-only -- see `current_role`'s own docstring for why this never
    calls `session.rollback()`.
    """
    require_known_resource_type(resource_type)
...
def active_grant_actions_for(
    ...
    *,
    ...
    here never touches it. Read-only -- see `current_role`'s own
    ...
    *why* access is allowed and exactly which actions it covers, rather
    ...
    if ctx.resource.visibility == "shared_explicitly":
        ...
        if account_id is None:
            return None
        actions = active_grant_actions_for(
            session,
...
def require_role_action(session: Session, auth: AuthContext, action: Action) -> None:
    ...
    `ROLE_PERMISSIONS` includes `read` today, so this is only ever
