...
def gmail_oauth_callback_endpoint(
    ...
    with SessionFactory() as create_session, create_session.begin():
        try:
            ...
            # A prior callback for this exact Google account already
            # connected it (a reloaded/duplicated callback request -- see
            # module docstring). Return the existing row's current state
            # rather than a hard error; this is a passive page-reload, not
            # a client retry that "earned" a fresh idempotent response.
            existing = create_session.execute(
                text(
            ...
            account = get_connector_account(create_session, auth.workspace_id, existing)
