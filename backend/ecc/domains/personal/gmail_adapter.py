...
class GmailAdapter:
    ...
    # account_allowed` above. Delegates to `gmail_action_detection.py`
    ...
    def detect_actions_since(
        ...
        *,
        ...
        """See `gmail_action_detection.detect_actions_since`'s own
        docstring for the full contract -- this method's own body is
        deliberately just the delegation described above.
        """
        from .gmail_action_detection import detect_actions_since as _detect_actions_since_impl

        _detect_actions_since_impl(self, context, since=since, ollama_adapter=ollama_adapter)
    ...
    def fetch_and_store_body(
        ...
        *,
        ...
        Originally extracted from `_detect_action_for_message` (Task 5,
        now in `gmail_action_detection.py` -- a later architecture-review
        split; it still calls this method, just from another module) so
        Task 6's `gmail_threads.py:get_thread_endpoint` (a second,
        cross-module caller -- the reason this method is not underscore-
        prefixed like every other single-caller helper in this file)
        reuses this exact fetch-and-store mechanism for its own
        on-demand, explicit-thread-open fetch, rather than a second one
        -- the phase plan's own "matching Task 5's own fetch-and-store
        path" Task 6 instruction. Callers are themselves responsible for
        checking `body IS NULL` before calling this (this method always
