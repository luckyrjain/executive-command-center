"""Connector-independent action-adapter contract
(`docs/superpowers/specs/2026-07-25-phase-5-automation-design.md` Decision
8, resolving `docs/phases/PHASE-REVIEW.md`'s F-03).

**This module defines the contract shape only** -- `ActionAdapter`,
`AdapterRegistry`, `TransientAdapterError`, and the two registration-error
types -- with no import of any concrete adapter implementation. It is
deliberately a leaf: nothing here imports from `ecc.domains.engineering`,
`local_adapters`, or any other adapter-implementing module, so any domain
that implements `ActionAdapter` (this activation's own `local_adapters.py`;
Phase 6's `engineering.write_actions`; any future adapter-implementing
domain) can depend on this module at import time with no risk of a cycle.

`ecc.domains.automation.adapters` is the separate **composition-root**
module: it imports this module's contract, imports every concrete adapter
implementation, and populates the one shared production `registry` instance
`ecc.domains.automation.worker` resolves a workflow step's `action_ref`
against. That split (architecture review, 2026-09-07, CAR-1) exists so an
adapter-implementing module like `engineering.write_actions` can import
`TransientAdapterError` at its own top level like any other dependency,
instead of the function-local workaround the two-module split replaces --
see `adapters.py`'s own module docstring for the composition-root side of
this, and `write_actions.py`'s module docstring for why the workaround it
...
class AdapterRegistry:
    ...
    needs, and the shared production registry (`adapters.registry`) is
    simply one particular instance the composition-root module populates.
    """
