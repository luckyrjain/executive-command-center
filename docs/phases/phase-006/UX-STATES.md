...
- **Partial permissions**: `permission_lost` at the connector-account level (`ConnectorHealthPanel`) and `permission_state` at the per-repository/per-work-item level (`RepositoriesPanel`/`WorkItemsPanel`, see the "Work items panel" note below) are both surfaced -- distinct scopes, since one connector account can have some repositories with lost permission and others still fully active.
