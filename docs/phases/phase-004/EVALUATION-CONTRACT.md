...
**Since superseded.** `.github/workflows/ollama-evaluation.yml` has since run for real, multiple times, against genuine `qwen2.5:1.5b-instruct-q4_K_M` output (PR #38's review cycle). The floors are not yet met -- most recently schema validity 90%, grounding 90%, 0 prohibited facts, p95 latency 17.3s vs. the required 100%/100%/0/<20s -- after two real fixes (markdown-fence stripping in `validator.py`, factor-code/source_field disambiguation in `runtime.py`'s prompt rendering) each measurably improved the metrics against real, evidence-driven failure patterns. The remaining gap is attributed to this activation's single 1.5B-parameter model's inherent capability, not a further defect in the harness or prompt path -- see `IMPLEMENTATION-STATUS.md`'s "What remains before Phase 4 itself can exit" for the full progression. No promotion decision has been made.
