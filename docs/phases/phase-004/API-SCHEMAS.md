...
When the Reflection Engine (first slice, `routing_policies.constraints.reflection_enabled`) is on for `attention.explain_item`, a `completed` run's `output`/`evidence` may transparently reflect a reflection-revised answer rather than the model's first-pass answer -- there is no separate response field indicating this happened. The underlying `ai_run_steps` record (not exposed by any endpoint in this activation, matching every other step kind) carries the full detail for operator/debugging purposes.
