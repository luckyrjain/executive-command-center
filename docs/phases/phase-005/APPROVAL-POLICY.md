...
Each action adapter statically declares its `high_impact_categories` at registration time, checked before any execution attempt. **An adapter that cannot classify itself into any category still defaults to requiring per-run approval** -- `bounded` (eligible for `bounded_recurring` mode, no per-run prompt) is a category an adapter must explicitly and correctly claim; there is no "none of the above, therefore bounded" default (fail closed, matching Phase 4's conservative data-class default precedent).
...
**Revocation** takes effect immediately for any not-yet-started step (matches the phase NFR verbatim); an in-flight step already dispatched completes or reaches its next durability checkpoint before the run halts.
