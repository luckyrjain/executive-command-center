...
- **PostgreSQL worker/lease design:** a lease-based worker inside the existing modular monolith, not Temporal (`RFC-005.md`'s pre-registered gate is evaluated and explicitly not activated this round, `ADR-0013`) -- 2-second poll interval, 30-second lease duration renewed by a heartbeat as each step is dispatched (`HEARTBEAT_INTERVAL_SECONDS = 10` is a declared constant, not a timer -- see `ADR-0013`'s own correction and `phase-005/DATA-MODEL.md`'s `workflow_runs` row), `sha256` `action_digest`-gated idempotent dispatch persisted before every side effect. Recovery is the same lease-expiry reclaim path a worker restart uses, comfortably inside the phase's own 60-second recovery target (`docs/runbooks/PHASE-5-RECOVERY.md`).
...
**Three items this document's own In scope / Data changes lines name were deliberately not built in this activation, recorded here so those lines are not read as delivered** (`docs/phases/phase-005/IMPLEMENTATION-STATUS.md` is the authority on delivered-versus-deferred, per slice; this is a pointer to its own disclosures, not a new decision):

- **Event triggers fire nothing.** "manual/event/schedule triggers" (In scope) shipped as manual and schedule only. An `event` trigger row is creatable and nothing auto-fires it -- the scheduler tick reads `trigger_type = 'schedule'` at the SQL level, and `platform/events/bus.py` still provides only `NonDurableInProcessEventBus` (`ADR-0013`'s own recorded outcome; `IMPLEMENTATION-STATUS.md`'s Task 4 evidence).
- **There is no `notifications` table.** "history/notifications" (In scope) and "and notifications defined in `phase-005/DATA-MODEL.md`" (Data changes) describe a designed shape, not a deployed table: no migration in `0038`-`0043` creates one and no code writes a notification row. `local.send_test_notification` returns an in-memory acknowledgement and persists nothing.
- **`secret_references` is likewise not built.** No migration creates the table; no adapter registered in this activation resolves a secret at all, and step payload protection rests on `worker._redact_payload`'s key-name redaction meanwhile (`phase-005/DATA-MODEL.md`'s own row). A Phase 6 connector needing a real credential is the point this has to become real.

One further limitation of the same kind, inside a capability that *did* ship: **compensation dispatch is not crash-resumable** -- a worker crash while a run is `status = 'compensating'` leaves it stuck requiring manual operator intervention (`docs/runbooks/PHASE-5-RECOVERY.md`'s own diagnostic guidance; `IMPLEMENTATION-STATUS.md`'s Task 6 disclosed scope boundary).
