...
- Any technology addition approved through RFC-005 and an ADR — not needed for Slices 1-6 and 8 (no new technology). **Resolved for Slice 7**: RFC-005 v1.2.0 activates `pgvector`/`sentence-transformers`, paired with ADR-0011, per the design doc's Open decision 2 and by explicit repository-owner authorization (see `docs/ROADMAP.md`'s Phase 2 status note).
- Versioned retrieval and entity-resolution evaluation datasets established — **done**: `tests/fixtures/phase2_resolution_dataset.py` (Slice 4), and `tests/fixtures/phase2_retrieval_embedding_dataset.py` (Slice 7, semantic-recall benchmark; see Slice 7 row).
...
| 7 | Optional embeddings and hybrid fusion | Backend implemented (commit `f7568d3`: migration `0015_phase2_embeddings.py` -- pgvector extension, `embedding_projections` table HNSW-indexed with `vector_cosine_ops`, plus the `(workspace_id, id)` unique constraint `retrieval_documents` needed for the composite FK; `embeddings.py` -- local `sentence-transformers/all-MiniLM-L6-v2` provider, lazy-loaded, gated by `Settings.embeddings_enabled` (default off), `queue_embedding`/`rebuild_embeddings` best-effort/degrade-not-fail; `retrieval.py`'s `GET /knowledge/retrieve?mode=hybrid` now genuinely fuses lexical and semantic candidates via a versioned deterministic formula preserving `RETRIEVAL-CONTRACT.md`'s ranking order, degrading to `degraded=true` lexical-only on any embedding failure; `pgvector/pgvector:pg18` replaces `postgres:18.0` in `docker-compose.yml` and both CI workflows), 20 passing tests (degraded-path-first ordering, a deterministic fake-provider suite for ranking-formula correctness, workspace isolation, migration reversibility) plus 2 real-model tests and the versioned semantic-recall benchmark (`tests/fixtures/phase2_retrieval_embedding_dataset.py`, `test_knowledge_retrieval_benchmark_postgres.py`, `MIN_HIT_AT_5_RATE` floor) that could not run in this sandbox (no network access to the model host) and are gated on CI as the authoritative verification -- see the PR for the actual CI result before treating the benchmark as passed; frontend: `EntityExplorer.tsx` now requests `mode=hybrid` (previously always lexical, leaving Task 8's degraded-state banner unreachable) |
