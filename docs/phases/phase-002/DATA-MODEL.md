...
|---|---|---|---|
| `knowledge_entities` | Canonical person, organization, project, topic, decision, document or team | id, workspace_id, kind, canonical_name, summary, status, version | `pkos_nodes`, extended with `status`, `confidence`, `version` (`node_type`/`canonical_name`/`attributes` already exist and map to `kind`/`canonical_name`/free-form fields; the originally-proposed `entity_id` column was dropped by migration `0020_phase2_drop_dead_entity_id.py` -- no shipped code path ever wrote a non-NULL value to it). **`team` (added post-Phase-6)**: same shape as every other kind, `canonical_name`/`summary` only -- no team-specific `attributes` schema exists yet (no kind has one; `attributes` is freeform JSONB with only `{"summary": ...}` ever written by any kind today). `entities_mutations.py`/`resolution.py`/`entity_operations.py`/`claims.py` needed zero changes -- all four are fully kind-agnostic already. **Update**: the "no cross-domain linkage exists yet" gap this paragraph originally named is now closed for `repositories`/`engineering_work_items` specifically -- migration `0050_phase6_team_linkage.py` (a later, separate follow-up change; the "hybrid: auto-suggest, human confirms" design it introduces is that migration's own, not something this document specified) added a `team_entity_id` composite FK plus a sync-populated `suggested_team_name` hint to both tables. `changes`/`reviews` still have no column of their own (they inherit the link transitively via `repository_id`/`change_id`), and the Phase 6 metrics-computation engine still does not read any of it -- see `docs/phases/phase-006/DATA-MODEL.md`'s and `DELIVERY-INTELLIGENCE-CONTRACT.md`'s own updated sections for the current, accurate state. **Accepted pre-existing gap, unrelated to this addition but worth naming here**: `resolution.py`'s `create_candidate` still inserts an `open` candidate row for a mismatched-kind pair (scored 0.0, never raising), and neither `confirm_candidate` nor `entity_operations.py`'s `merge_entities` re-checks `left.kind == right.kind` before confirming/merging -- a human (or buggy client) confirming a score-0 candidate could merge two different-kind entities. Not new or specific to `team`, but adding a kind whose real-world names commonly collide with `organization`/`project` (e.g. "Platform Engineering" as both a team and an org) makes this pre-existing gap more likely to actually fire. A real fix (a kind-equality guard in `confirm_candidate`/`merge_entities`) is deliberately out of scope for this addition -- it touches core merge-safety logic for every kind pair, not just `team`, and deserves its own properly-scoped task. |
| `entity_aliases` | Names and external identifiers | entity_id, alias_type, normalized_value, source_id, confidence | new table, `entity_id` FK to `pkos_nodes`. **Accepted gap at Phase 2 delivery, since closed by a later phase**: no Phase-2-owned endpoint ever inserted a row here -- `GET /entities/{id}/aliases` (list) is the only Phase 2 HTTP surface, merge's `_rehome_aliases` only moves existing rows between entities. Phase 10's Gmail connector (`personal/gmail_adapter.py`) now writes real rows during backfill/incremental sync, so resolution scoring's alias-overlap factor and retrieval's exact-alias-match tier can fire in a real deployment with Gmail connected -- the table is no longer guaranteed permanently empty. No Phase-2-owned "attach an alias/external identifier" user-facing endpoint exists yet; see `IMPLEMENTATION-STATUS.md` for tracking. |
