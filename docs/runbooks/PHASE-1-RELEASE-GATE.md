...
**Baseline:** `feature/phase-1-production-hardening`, commit `7072b1c` (Task 12's documentation-synchronization commit `0edfa37`, the standalone CI-secret fix `87e12b2`, and a follow-up runbook correction). **This baseline is stale** — multiple review/fix rounds have landed on the branch since (rate-limiter proxy-IP handling, dashboard/brief cache invalidation, `dashboard_briefs.py` correctness fixes, dependency bumps, the Alpine base-image switch, and more). Treat every commit-SHA-anchored claim below as describing that older baseline, not the branch's current state, until this document is re-synchronized against current HEAD.
...
> document's stated evidence for it cannot currently be inspected.
...
- [ ] Dependency, secret, container and SBOM scans pass. (Task 12 originally found real findings locally against an older commit on this branch: `pip-audit` clean; Trivy filesystem HIGH findings in `pnpm-lock.yaml`'s `react-router`; `pnpm audit --audit-level=high` findings including `react-router`/`vite`; Trivy image findings in both base images, mostly OS-package. Status now unverified rather than confirmed-FAIL: this branch has since bumped `react-router` to `7.18.1` (commit `c4ca876`, above the `>=7.15.0` threshold Task 12 itself cited as the fix) and switched the backend base image from `python:3.14.6-slim` to `python:3.14.6-alpine` with an explicit `apk upgrade` step (commit `8ebb32d`; the frontend's `nginx:1.27-alpine` image was unchanged). Neither change has been re-scanned -- do not treat the old failing numbers, or an assumption the bumps fully resolved them, as current evidence. gitleaks and the SBOM step were not independently re-run locally either time. Re-run the `containers`/`security`/`frontend` CI jobs against current HEAD for a real answer.)
