...
- `sentence-transformers` pulls in `torch` as a transitive dependency — a multi-hundred-MB addition to the backend's dependency footprint, the largest single dependency this codebase has added to date. Accepted as the cost of genuine local-first inference (the alternative, an external embedding API, was rejected below).
- `torch` publishes no musl/Alpine-compatible wheels at all, and `backend/Dockerfile` is deliberately Alpine-based for a smaller, more hardened production image (a Phase 1 decision this ADR does not reverse). Resolved by making `sentence-transformers` an optional dependency group (`embeddings`, `pyproject.toml`) not installed by the production image's `uv sync --frozen --no-dev` — mirroring `Settings.embeddings_enabled`'s own off-by-default design at the packaging level, not just the runtime-config level. `pgvector` itself stays a required dependency: it's pure-Python (no musl issue) and migration `0015` imports `pgvector.sqlalchemy.Vector` at module level, which `alembic upgrade head` needs importable in every environment regardless of whether embeddings are ever turned on. A deployment that wants Task 7 live builds a custom (non-Alpine, or Alpine plus a manually-built musl-incompatible-free alternative) image with `uv sync --extra embeddings`; CI's dedicated `embeddings-benchmark` job (native Linux glibc, isolated from `backend`'s tight-budget Phase 1 performance tests to avoid the resource contention a torch install alongside them caused) does exactly this to run the real-model tests below.
