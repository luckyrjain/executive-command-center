...
- `ecc.domains.identity.accounts` (placement finalized during implementation, not `ecc.platform.accounts` as first sketched here: account/login/workspace/invitation code is a real product capability with its own business meaning, not pure cross-domain infrastructure with no meaning of its own the way the event bus or the future `authorize()` engine are -- it belongs in `ecc/domains/<name>/` per this repository's own `platform/` vs `domains/` distinction, alongside the existing `person_organizations.py` in the same `identity` domain package): Argon2id hashing, `POST /accounts` (shipped in Task 1 as genuine open self-registration, not yet invitation-token-gated -- the `invitations` table is Task 2's own table, so gating is added once it exists rather than shipping an endpoint that accepts a token parameter it cannot yet validate; see Task 2), `POST /auth/login`, `POST /auth/select-workspace`, reusing `backend/ecc/auth.py`'s existing `ecc_session`/`ecc_csrf` cookie mechanism unchanged in shape.
