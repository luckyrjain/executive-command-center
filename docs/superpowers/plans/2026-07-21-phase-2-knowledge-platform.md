...
- [ ] **Step 3: Write failing pipeline tests:** exact identifier/alias match ranks above lexical relevance; workspace/permission/lifecycle/time filters; signed-cursor pagination and tamper rejection (reuse Phase 1's existing cursor-signing utility, do not reimplement); `degraded=false` when embeddings aren't requested (mode is always `lexical` until Slice 7 exists).
