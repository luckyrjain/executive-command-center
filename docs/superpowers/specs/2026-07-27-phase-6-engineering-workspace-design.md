...
- `ecc.domains.engineering.connectors.ConnectorAdapter` (a `typing.Protocol`, mirroring `ecc.domains.automation.adapters.ActionAdapter`'s shape) declaring `authorize`, `validate`, `backfill`, `incremental_sync`, `handle_webhook`, `refresh_permissions`, `disconnect`, plus `ConnectorRegistry`.
