...
def _load_bootstrap_module() -> ModuleType:
    script_path = Path(__file__).resolve().parents[1] / "scripts" / "bootstrap_dev.py"
    spec = importlib.util.spec_from_file_location("bootstrap_dev", script_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load scripts/bootstrap_dev.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


bootstrap_dev = _load_bootstrap_module()
