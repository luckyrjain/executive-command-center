...
def test_post_evaluations_runs_happy_path(run_context: dict, http_client: TestClient) -> None:
    response = http_client.post(
        ...
        json={"task_type": TASK_TYPE, "prompt_version": 3, "model_id": _SEEDED_MODEL_ID},
...
def test_post_evaluations_runs_conflicting_payload_same_key_returns_idempotency_conflict(
    ...
    conflicting = http_client.post(
        "/api/v1/ai/evaluations/runs",
        json={"task_type": TASK_TYPE, "prompt_version": 3, "model_id": _SECOND_SEEDED_MODEL_ID},
        headers=headers,
    )
...
def test_post_evaluations_runs_unknown_model_is_422(
    ...
    response = http_client.post(
        "/api/v1/ai/evaluations/runs",
        json={"task_type": TASK_TYPE, "prompt_version": 3, "model_id": "not-registered"},
...
def test_activate_gated_prompt_rejected_with_no_passing_evaluation(
    ...
    gate must reject even a reactivation of the already-active version 3
    (migration `0055_phase4_expl_item_prompt_v3.py`'s activated row).
    """
    response = http_client.post(
...
def test_activate_gated_prompt_allowed_once_evaluation_passes(
    ...
    assert activate_response.json()["active_version"] == 3
