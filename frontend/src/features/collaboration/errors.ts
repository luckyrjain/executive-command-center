...
export function collaborationErrorMessage(error: unknown): string {
  ...
  if (error.code === 'INVITATION_TOKEN_INVALID') return 'That invitation ID and token don\'t match. Double-check both and try again.'
  if (error.code === 'INVITATION_EXPIRED') return 'This invitation has expired. Ask for a new one.'
  if (error.code === 'INVITATION_EMAIL_MISMATCH') return 'This invitation was sent to a different email address than your account\'s.'
  if (error.code === 'INVITER_NO_LONGER_AUTHORIZED') return 'The person who sent this invitation no longer has the authority to grant that role. Ask an owner to invite you again.'
