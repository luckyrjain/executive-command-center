  ...
  return <section className="work-panel" aria-labelledby="risks-title">
    ...
    <ol className="work-list">{(query.data?.items ?? []).map((value) => {
      ...
      return <li key={value.id}>
        ...
        <details>
          <summary>Details for {value.description}</summary>
          <dl className="detail-fields">
            <div><dt>Owner</dt><dd className="is-machine-value">{value.owner_id}</dd></div>
            <div><dt>Score rationale</dt><dd>{value.explanation || '—'}</dd></div>
            <div><dt>Mitigation</dt><dd>{value.mitigation || '—'}</dd></div>
            <div><dt>Trigger</dt><dd>{value.trigger || '—'}</dd></div>
          </dl>
        </details>
