...
type Draft = { summary: string; description: string; direction: Commitment['direction']; counterpartyName: string; importance: Commitment['importance']; dueDate: string; dueAt: string }
...
function fromCommitment(value: Commitment): Draft {
  return { summary: value.summary, description: value.description ?? '', direction: value.direction, counterpartyName: value.counterparty_name ?? '', importance: value.importance, dueDate: value.due_date ?? '', dueAt: value.due_at?.slice(0, 16) ?? '' }
}
...
export default function CommitmentWorkspace() {
  ...
  const editMutation = useMutation({
    ...
    onError: (error) => {
      ...
      const current = error.current as Commitment | undefined
  ...
  return <section className="work-panel" aria-labelledby="commitments-title">
    ...
    <ol className="work-list">{(query.data?.items ?? []).map((value) => {
      ...
      return <li key={value.id}><div><strong>{value.summary}</strong><small>{value.direction.replaceAll('_', ' ')} · {value.counterparty_name ?? 'No counterparty'} · {value.status}</small></div><div className="work-actions" aria-label={`Actions for ${value.summary}`}>
        {!archived && !terminal ? <button type="button" aria-label={`Edit ${value.summary}`} onClick={() => setEdit({ commitment: value, ...fromCommitment(value), latestVersion: value.version, conflict: false })}>Edit</button> : null}
        {!archived && value.status === 'detected' ? <button type="button" aria-label={`Confirm ${value.summary}`} onClick={() => actionMutation.mutate({ commitment: value, action: 'confirm' })}>Confirm</button> : null}
        {!archived && ['confirmed', 'active'].includes(value.status) ? <button type="button" aria-label={`Fulfil ${value.summary}`} onClick={() => actionMutation.mutate({ commitment: value, action: 'fulfil' })}>Fulfil</button> : null}
        {!archived && !terminal ? <button type="button" aria-label={`Cancel ${value.summary}`} onClick={() => actionMutation.mutate({ commitment: value, action: 'cancel' })}>Cancel</button> : null}
        {!archived ? <button type="button" aria-label={`Archive ${value.summary}`} onClick={() => actionMutation.mutate({ commitment: value, action: 'archive' })}>Archive</button> : <button type="button" aria-label={`Restore ${value.summary}`} onClick={() => actionMutation.mutate({ commitment: value, action: 'restore' })}>Restore</button>}
      </div></li>
    })}</ol>
    {edit ? <form onSubmit={submitEdit}><h2>Edit commitment</h2>
      ...
      {edit.conflict ? <button type="button" onClick={() => submitEdit()}>Retry with latest version</button> : <button type="submit">Save commitment</button>}
