...
def patch_events() -> None:
    path = Path("backend/ecc/domains/calendar/events.py")
    text = path.read_text()
    marker = "\n\ndef _get_row(\n"
    helper = dedent(
        """
        ...
        def _write_audit_and_outbox(
            ...
            session.execute(
                ...
                ),
                {
                    ...
                    "correlation_id": correlation_id,
                    "payload": dumps(
                        {"calendar_event_id": str(event_id), "version": version}
                    ),
                    "occurred_at": now,
                },
            )
    ...
    if "def _write_audit_and_outbox(" not in text:
    ...
    text = text.replace(
        ...
        "response = CalendarEventResponse.model_validate(row)\n"
        "        _write_audit_and_outbox(\n"
        "            session, auth, request, 'calendar_event.updated', event_id,\n"
        "            response.version, sorted(fields), now\n"
        "        )\n"
        "        _store_cached(session, auth, idempotency_key, request_hash, response, 200, now)\n"
        "        return response\n\n\ndef _lifecycle(",
    )
    ...
    text = text.replace(
        ...
        "        _write_audit_and_outbox(\n"
        "            session, auth, request, event_type, event_id, response.version,\n"
...
def patch_meetings() -> None:
    ...
    helper = dedent(
        ...
        def _encode_cursor(updated_at: datetime, meeting_id: UUID) -> str:
            ...
            signature = new(
                get_settings().session_secret.encode(), payload, "sha256"
            ).hexdigest().encode()
    ...
    text = text.replace(
        ...
        "        items=[_project(session, auth, dict(row)) for row in rows]\n"
        "    )",
        "    page = rows[:limit]\n"
        "    next_cursor = None\n"
        ...
        "    return MeetingListResponse(\n"
        "        items=[_project(session, auth, dict(row)) for row in page],\n"
