...
- **No single dominant anchor on the Today dashboard or `EngineeringOverview.tsx`** — both render several equal-weight `.dashboard-card`s (plus, on the dashboard, a separate `MorningBrief` panel) with no visual hierarchy between them, which "Building a new page"'s Hierarchy rules above name directly as the anti-pattern to avoid. Fixing this means deciding which section actually is the primary anchor on each page — a real design decision, not a class to add.
