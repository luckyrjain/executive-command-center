...
- **Three more unwizardized 5+-field forms**, beyond the already-documented Task/Commitment exceptions above: `WaitingView.tsx` (5 fields), `RecordsPanel.tsx` (a dynamic field array — the structural-complexity trigger, not just the field-count one), and `DelegationsPanel.tsx`'s propose-delegation form (5 fields). Whether each should become a real wizard (stepper, review step, terminal validation) is a per-form product call, not something to improvise silently.
- **No single dominant anchor on the Today dashboard** (`App.tsx`) — it renders several equal-weight `.dashboard-card`s inside `.dashboard-grid`, plus a separate `MorningBrief` panel below, with no visual hierarchy between any of them. This is the exact anti-pattern "Building a new page"'s Hierarchy rules name above ("two `.work-panel`s of equal size fighting for attention"). Fixing it means deciding which section actually is the primary anchor — a real design decision, not a class to add.
- **A milder, related issue in `EngineeringOverview.tsx`** — this one *is* a single `.work-panel` (one real anchor at the page level, not a competing-panels problem), but its `.work-list` gives all 4 summary rows (Connectors, Open incidents, Open decisions, headline metrics) identical visual weight, with nothing indicating which is most decision-relevant right now. Smaller than the dashboard's problem and arguably fine as-is for a glance-summary panel; noted here rather than fixed because which row (if any) should be promoted is a product call, not a conformance bug.
