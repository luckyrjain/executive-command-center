...
Styled by `.workspace-nav` (`display: flex; flex-wrap: wrap; gap: 8px`) — until Visual Foundation v2's navigation sub-project, the tablist `<div>` had no `className` at all, so the app's only selected-state rule at the time (`.tab-list button[aria-selected="true"]`) never matched it and every pill looked identical regardless of which page was active. `.workspace-nav button[aria-selected="true"]` is the app's first real consumer of `--color-accent` — filled with the accent rather than `.tab-list`'s ink fill, so the one deliberate accent has a single, meaningful job (this nav's selected state) instead of being reused for everything. Deliberately doesn't get `.tab-list`'s mobile column-stack treatment (Responsive behavior below) — that fits `.tab-list`'s typical 2-4 tabs; forcing this nav's 15 tabs into one vertical column would be worse than the wrapped rows `flex-wrap` already gives it at any width.
